...
export const EventCard = ({ title, description, image, gradient, icon = "music" }: EventCardProps) => {
  ...
  return (
    <Card className="group relative overflow-hidden border-border/50 bg-card/50 backdrop-blur-sm hover:shadow-glow-cyan transition-all duration-500 hover:scale-[1.02]">
      ...
      {/* Content */}
      <div className="p-6 sm:p-6 md:p-8 space-y-4 md:space-y-4">
        <div className="flex items-center gap-3 md:gap-3">
          <div className="p-2.5 md:p-3 rounded-lg bg-gradient-primary">
            <Icon className="h-5 w-5 md:h-6 md:w-6" />
          </div>
          <h3 className="text-2xl sm:text-2xl md:text-3xl font-bold" style={{ fontFamily: 'Orbitron, sans-serif' }}>
            {title}
          </h3>
        </div>
        
        <p className="text-base sm:text-base md:text-lg text-muted-foreground leading-relaxed" style={{ fontFamily: 'Rajdhani, sans-serif' }}>
          {description}
        </p>

        {/* Decorative line */}
        <div className="pt-3 md:pt-4">
          <div className="h-1 w-20 md:w-20 bg-gradient-primary rounded-full" />
        </div>
      </div>
    </Card>
  );
